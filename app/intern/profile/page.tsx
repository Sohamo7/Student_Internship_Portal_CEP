...
export default function InternProfilePage() {
  ...
  const [bio, setBio] = useState('Passionate about social development, youth literacy, and education technology.');
      ...
      <main className="flex-1 p-6 md:p-8 space-y-6 max-w-4xl">
        <div>
          ...
          <p className="text-sm text-slate-600">
            View and manage your active internship identity and account preferences.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200/90 bg-white p-6 md:p-8 shadow-xs">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-teal-600 text-white font-black text-2xl shadow-md shadow-teal-600/20">
              {profile?.name ? profile.name.charAt(0) : 'A'}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold text-slate-900">{profile?.name || 'Aarav Patel'}</h2>
                <span className="rounded-md bg-teal-100 px-2 py-0.5 text-xs font-bold text-teal-800">
                  Active Intern
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">{profile?.email || 'intern@ngo.org'}</p>
            </div>
          </div>
