...
export default function InternApplicationPage() {
  ...
  const [college, setCollege] = useState(profile?.college || 'National Institute of Technology (NIT)');
  const [degree, setDegree] = useState(profile?.degree || 'B.Tech in Computer Science');
  const [interest, setInterest] = useState(profile?.program_interest || 'Community Digital Literacy & Youth Education');
  const [sop, setSop] = useState(profile?.statement_of_purpose || 'I want to volunteer my coding and digital skills to help teach basic computer literacy to underprivileged students.');
  ...
  return (
    <div className="flex flex-1 flex-col md:flex-row bg-slate-50/60">
      ...
      <main className="flex-1 p-6 md:p-8 space-y-6 max-w-5xl">
        ...
        {/* Status Callout */}
        <div className="rounded-2xl border border-teal-200 bg-white p-6 shadow-xs">
          <div className="flex items-start gap-4">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-teal-600">
              <CheckCircle2 className="h-6 w-6" />
            </div>
            <div>
              <h2 className="text-base font-bold text-slate-900">Application Approved</h2>
              <p className="mt-1 text-xs text-slate-600 leading-relaxed">
                Your internship application has been approved by <strong>Navodaya Social Development Trust</strong>. Your active project and deliverables are now assigned.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200/90 bg-white p-6 md:p-8 shadow-xs">
          ...
          <form onSubmit={handleUpdate} className="space-y-5">
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div>
                ...
                <input
                  type="text"
                  disabled
                  value={profile?.name || 'Aarav Patel'}
              ...
              <div>
                ...
                <input
                  type="email"
                  disabled
                  value={profile?.email || 'intern@ngo.org'}
                  className="mt-1.5 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700 font-medium"
                />
              </div>
            ...
            <div className="pt-2 flex items-center justify-between">
              <span className="text-xs text-slate-500">Resume: <code className="text-indigo-600 font-mono">aarav_intern_resume.pdf</code></span>
              <button
                type="submit"
